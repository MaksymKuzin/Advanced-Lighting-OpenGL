...
inline unsigned int TextureFromFile(const char *path, const std::string &directory, const aiScene* scene, bool gamma) {
    ...
    if (filename.c_str()[0] == '*') {
        std::string indexStr = filename.substr(1);
        int tIndex = std::stoi(indexStr);
        aiTexture* texture = scene->mTextures[tIndex];

        if (texture->mHeight == 0) {
            // Текстура стиснена (png/jpg) і лежить в пам'яті
            data = stbi_load_from_memory(reinterpret_cast<unsigned char*>(texture->pcData), texture->mWidth, &width, &height, &nrComponents, 0);
        }
    } else {
        // Якщо раптом текстура зовнішня
